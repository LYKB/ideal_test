...
<div class="pt-5 pb-5 gray-simple">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-8 col-md-1 col-sm-12">
            ...
            <div class="col-lg-8 col-md-12">
                    ...
                    <div id="clOne" class="panel-collapse collapse show">
                        <div class="block-body">
                            <ul class="deatil_features">
                                ...
                                <li><strong>厨房数量：</strong>${house.kichenNum}个</li>
                                <li><strong>房间、房屋面积：</strong>${house.area}平米</li>
                                <li>
                                    <strong>是否有空调：</strong>
                                    <c:choose>
                                        <c:when test="${house.hasAirConditioner == 1}">有空调</c:when>
                                        <c:otherwise>没有空调</c:otherwise>
                                    </c:choose>
                                </li>
                ...
                <c:if test="${house.rentType == 'share'}">
                    <div class="property_block_wrap style-2" id="shareHouse">
                        <div data-toggle="collapse" href="#clThree" class="property_block_wrap_header">
                            <h4 class="property_block_title">合租情况</h4>
                        </div>
                                                            ...
                                                            <c:choose>
                                                                <c:when test="${c.currentOrder!=null}">
                                                                    <div class="comment-text">
                                                                        ...
                                                                        <p>入职日期：<fmt:formatDate value="${c.currentOrder.startDate}" pattern="yyyy-MM-dd"></fmt:formatDate></p>
                                                                    </div>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <div class="comment-text">暂时租出</div>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </div>
                                                    </article>
                ...
                <div class="property_block_wrap style-2">
                    <div data-toggle="collapse" href="#clFive" class="property_block_wrap_header">
                        <h4 class="property_block_title">房子照片</h4>
                    </div>
                                    ...
                                    <div class="form-group">
                                        <c:choose>
                                            <c:when test="${house.status == 0}">
                                                <a href="javascript:void(0);" class="btn btn-md full-width btn-theme bg-2" onclick="createOrder()">立即预定</a>
                                            </c:when>
                                            <c:when test="${house.status == 1}">
                                                <a href="javascript:void(0);" class="btn btn-md full-width btn-theme bg-red" disabled>已租出</a>
                                            </c:when>
                                            <c:when test="${house.status == -1}">
                                                <a href="javascript:void(0);" class="btn btn-md full-width btn-theme bg-red" disabled>已下架</a>
                                            </c:when>
                                            <c:when test="${house.status == -2}">
                                                <a href="javascript:void(0);" class="btn btn-md full-width btn-theme bg-red" disabled>待审核</a>
                                            </c:when>
                                            <c:when test="${house.status == -3}">
                                                <a href="javascript:void(0);" class="btn btn-md full-width btn-theme bg-red" disabled>审核不通过</a>
                                            </c:when>
                                        </c:choose>
                                    </div>
                                </div>
                        ...
                        <div class="side-block-body">
                            <form id="contactForm">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-6">
                                        <div class="form-group">
                                            <textarea class="form-control" name="content" placeholder="想要发的信息内容"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-lg-12 col-md-12 col-sm-6">
                                        <div class="form-group">
                                            <a href="javascript:void(0)" onclick="submitContact()" class="btn btn-md full-width btn-theme bg-2">发送邮件信息</a>
                                        </div>
                                    </div>
                                </div>
