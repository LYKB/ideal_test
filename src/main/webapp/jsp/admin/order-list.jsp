                                    ...
                                    <td class="dashboard_propert_wrapper">
                                        ...
                                        <div class="title">
                                            ...
                                            <span class="table-property-price">
                                                ...
                                                到期时间：<fmt:formatDate value="${c.endDate}" pattern="yyyy-MM-dd"></fmt:formatDate>
                                            </span>
                                            <span></span>
                                            <span class="table-property-price">
                                                总金额： ￥${c.totalAmount}
                                            ...
                                            <c:choose>
                                                <c:when test="${c.status == -3}"><span class="text-warning">租客已取消</span> </c:when>
                                                <c:when test="${c.status == -2}"><span class="text-warning">待签合同</span> </c:when>
                                                <c:when test="${c.status == -1}"><span class="text-warning">待付款</span> </c:when>
                                                <c:when test="${c.status == 0}"><span class="text-warning">生效中</span> </c:when>
                                                <c:when test="${c.status == 1}"><span class="text-warning">已到期</span> </c:when>
                                                <c:when test="${c.status == 2}"><span class="text-warning">退租申请</span> </c:when>
