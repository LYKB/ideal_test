...
public class HouseController extends BaseController {
    ...
    public String houseDetail(@PathVariable("id")Long id, Model model){
        ...
        //如果是合租，查询合租房子
        List<House> shareHouseList = houseService.findByUserIdAndCetficateNoAndRentType(house.getUserId(),house.getCetificateNo(), HouseRentTypeEnum.SHARE.getValue());
        //从订单里查找到租户id，吧租户放到每个房子里
    ...
    public String map(@RequestParam("id")Long id,Model model){
        ...
        House house = houseService.get(id);
        if (house == null){
            return renderNotFound();//没有房子返回没有找到
        }
